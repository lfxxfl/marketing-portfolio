...
import styles from './ripple-distortion.module.css';
...
type Uniform = { value: number | number[] | Texture };
type UniformGroup = Record<string, Uniform>;
...
export function RippleDistortion({
  ...
  const uniformsRef = useRef<{ wave: UniformGroup; composite: UniformGroup } | null>(null);
  ...
  useEffect(() => {
    ...
    const waveUniforms: UniformGroup = { uRings: { value: 4 } };
    ...
    const loop = (now: number) => {
      ...
      const offsetAttribute = geometry.attributes.iOffset;
      const scaleAttribute = geometry.attributes.iScale;
      const opacityAttribute = geometry.attributes.iOpacity;
