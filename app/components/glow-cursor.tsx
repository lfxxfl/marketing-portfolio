...
import styles from './glow-cursor.module.css';
...
type GlowConfig = Required<Omit<GlowCursorProps, 'children' | 'className' | 'style'>>;
...
export function GlowCursor({
  ...
  const configRef = useRef<GlowConfig>({
    color, secondaryColor, trailLength, trailWidth, trailTaper, followSpeed, glowIntensity,
    glowSpread, hotspot, brightness, opacity, pulseSpeed, noiseStrength, idleFade, idleTimeout,
    fadeDuration, blendMode, maxDevicePixelRatio, enabled,
  });
  ...
  }, [brightness, color, enabled, fadeDuration, followSpeed, glowIntensity, glowSpread, hotspot, idleFade, idleTimeout, maxDevicePixelRatio, noiseStrength, opacity, pulseSpeed, secondaryColor, trailLength, trailTaper, trailWidth, blendMode]);
  ...
  useEffect(() => {
    ...
    const render = (now: number) => {
      ...
      if (initialized) {
        const headEase = 1 - Math.pow(1 - clamp(config.followSpeed, .01, .99), delta);
      ...
      uniforms.uPointCount.value = clamp(Math.round(config.trailLength), 2, MAX_POINTS);
      uniforms.uColor.value = hexToRgb(config.color);
      uniforms.uSecondaryColor.value = hexToRgb(config.secondaryColor);
      uniforms.uTrailWidth.value = Math.max(config.trailWidth, .1);
  ...
  return (
    <div ref={containerRef} className={`${styles.root}${className ? ` ${className}` : ''}`} style={style} aria-hidden={!children}>
      <canvas ref={canvasRef} className={styles.canvas} style={{ mixBlendMode: blendMode }} aria-hidden="true" />
